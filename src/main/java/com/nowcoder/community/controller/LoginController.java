...
public class LoginController implements CommunityConstant {
    ...
    public String register(Model model, User user) {
        ...
        if (map == null || map.isEmpty()) {
            //注册成功，提示信息，然后跳转到首页
            model.addAttribute("msg","注册成功，我们已经向您的邮箱发送了一封激活邮件，请尽快激活!");//向中间页面operate-result.html 发送一个msg。在这个html页面中由js实现了几秒后自动跳到某个页面
    ...
    public String activation(Model model, @PathVariable("userId") Integer userId,
        ...
        if(result==ACTIVATION_SUCCESS){
            ...
            model.addAttribute("msg","激活失败，您提供的激活码有问题！");
            model.addAttribute("target","/index");
        }
        return "/site/operate-result";
    }
